...
HW2b::initializeGL()
{
	...
	// DO NOT NEED TO SET MODELVIEW TO IDENTITY CAUSE IT IS DONE IN CONSTRUCTOR
...
HW2b::paintGL()
{
	...
	// Now we have to pass the uniforms to the vertex shader
	// projection matrix
	// modelview matrix
	// Theta
	// Twist
	...
	// draw triangles
	glDrawArrays(GL_TRIANGLES, 0, (GLuint) m_numPoints);
...
HW2b::initVertexBuffer()
{
	...
	// Generate buffers using member variables
	...
	// Bridge to the gpu
	// HW.cpp binds the "a_Position" attribute with the typedef ATTRIB_VERTEX
	// So we enable it and tell it that it will be recieving two floats
	...
	// Bridge to the gpu here as well
	// HW.cpp binds the "a_Color" attribute with the typedef ATTRIB_COLOR
	// So we enable it and tell it that it will be recieving three floats
...
{
	// PUT YOUR CODE HERE
	// This is the same code as HW1b 
	// Since the divisions are then stored in m_points and colors are stored in m_colors when triangle() is called
