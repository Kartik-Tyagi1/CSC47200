...
HW2a::resizeGL(int w, int h)
{
	...
	// This time we use the projection matrix instead of legacy method
...
HW2a::paintGL()
{
	...
	/* 
	 * initVertextBuffer() already loaded the points to the GPU
	 * So all we need to do is load the program using the enum at the top 
	*/
	
	glUseProgram(m_program[HW2A].programId());

	// pass the projection matrix to vertex the shader since we set it to an orthographic projection
	...
	for(int i = 0; i < 9; i++){
		...
		// No need for extra for loop becuase we send all verticies at once
		// So we just need to loop through the draw modes
	...
	/*
	 * Unlike for HW0C and what we will do for HW2B, we are not doing anything else to the veritices
	 * No rotations, scaling etc.
	 * So we don't need to worry about binding the attributes
	 * So we don't need to disable them at the end, only the program
	*/
